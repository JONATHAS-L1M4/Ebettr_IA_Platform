...
export const ClientUsers: React.FC = () => {
 ...
 <TableHead className="px-6 py-4">Empresa / Grupo</TableHead>
 ...
 const isBlocked = client.status === 'blocked';
 return (
 <TableRow key={client.id || client.email} className={`hover:bg-muted transition-colors group ${isBlocked ? 'bg-red-950/40' : ''}`}>
 ...
 <button type="submit" disabled={!newEmail || !newName || !newCompany || isSubmitting} className="mt-2 flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-xs font-bold uppercase tracking-wide text-muted-foreground shadow-sm transition-all hover:border-border hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50">

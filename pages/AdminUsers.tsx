...
const TableSkeleton = () => (
 ...
 <div className="h-12 bg-muted border-border flex items-center px-6">
...
export const AdminUsers: React.FC<AdminUsersProps> = ({ currentUserEmail }) => {
 ...
 {/* Header */}
 <div className="flex flex-col gap-4 border-border pb-4 mb-8">
 ...
 <TableHeader>
 <TableRow className="bg-muted border-border text-xs font-bold text-muted-foreground uppercase tracking-wider">
 <TableHead className="px-6 py-4">Usuário</TableHead>
 <TableHead className="px-6 py-4">Perfil</TableHead>
 ...
 return (
 <TableRow key={user.id || user.email} className={`hover:bg-muted transition-colors group ${isBlocked ? 'bg-red-950/40' : ''}`}>
 ...
 <button type="button" onClick={() => { setNewRole('admin'); setIsRoleDropdownOpen(false); }} className="flex h-10 w-full items-center gap-2 border-border px-3 py-2 text-left text-sm text-muted-foreground transition-colors hover:bg-muted"><Shield className="w-3.5 h-3.5 text-muted-foreground" /><span>Administrador</span>{newRole === 'admin' && <CheckCircle2 className="w-3.5 h-3.5 text-green-500 ml-auto" />}</button>
 ...
 <button type="submit" disabled={!newEmail || !newName || isSubmitting} className="mt-2 flex h-10 w-full items-center justify-center gap-2 rounded-lg border border-border bg-card px-4 py-2 text-xs font-bold uppercase tracking-wide text-muted-foreground shadow-sm transition-all hover:border-border hover:bg-muted hover:text-foreground disabled:cursor-not-allowed disabled:opacity-50">

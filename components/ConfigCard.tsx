...
import { Brain, Settings, MessageSquare, Shield, Database, CheckCircle2, Pencil, Trash2, CircleHelp, X, RotateCcw, Cpu, Cloud, Code, Zap, Globe, Activity, Terminal, Check, Key, Lock, ChevronDown, ChevronUp, AlertTriangle, Layers, Box, User, Mail, Loader2, ArrowUp, ArrowDown } from './ui/Icons';
...
const ConfigCard: React.FC<ConfigCardProps> = ({ section, onEdit, onDelete, onMoveUp, onMoveDown, onSaveSection, onWidthChange, isAdmin, isLocked = false, variant = 'default' }) => {
        ...
        return (
            <div className="relative">
                ...
                <input 
                    ...
                    readOnly 
                    className="w-full pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-md text-gray-500 text-sm font-mono shadow-inner cursor-not-allowed select-all"
                />
            </div>
        );
    }
  ...
  return (
    <>
      <SpotlightCard 
          enableOverflow={true}
          className={`group/card flex flex-col h-full border-gray-200 hover:border-gray-300 ${isLocked ? 'opacity-85' : ''}`}
      >
        
        {/* Card Header Padronizado */}
        <div className="p-6 pb-2 flex items-start gap-4 justify-between relative z-20">
          <div className="flex items-center gap-4 min-w-0">
              <div className="w-10 h-10 bg-gray-50 border border-gray-200 rounded-lg flex items-center justify-center text-black shrink-0 relative">
                {getIcon()}
              </div>
              <div className="min-w-0 pt-0.5">
                <h3 className="text-sm font-bold text-gray-900 tracking-wide uppercase truncate leading-tight">{section.title}</h3>
                <p className="text-[11px] text-gray-500 mt-1 font-medium leading-tight">{section.description}</p>
              </div>
          </div>
                    ...
                    <fieldset disabled={isFieldDisabled} className="contents">
                        ...
                        <div className="flex justify-between items-center mb-1.5">
                            <div className="flex items-center gap-2">
                                <label className="text-xs font-semibold text-gray-600 block uppercase tracking-wide flex items-center gap-1">
                                    {field.label}
                                    {field.required && <span className="text-red-500">*</span>}
                                    {field.secret && <span title="Campo Secreto"><Lock className="w-2.5 h-2.5 text-gray-400 ml-1" /></span>}
                                    {field.id === 'n8n_workflow_id' && <span className="text-[9px] text-gray-400 bg-gray-50 px-1 rounded ml-1 border border-gray-100">AUTO</span>}
                                </label>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                                {field.helpText && (
                                    <button 
                                        ...
                                        className="text-gray-300 hover:text-black transition-colors cursor-pointer outline-none"
                                ...
                                {isModified && !isLocked && field.id !== 'n8n_workflow_id' && field.type !== 'uuid' && (
                                    <div className="relative">
                                        <button 
                                            ...
                                            className={`transition-all transform hover:text-black ${isConfirmingReset ? 'text-black rotate-180' : 'text-gray-300 hover:rotate-[-45deg]'}`}
                                        ...
                                        {isConfirmingReset && (
                                            <div className="absolute right-0 top-6 z-50 bg-white border border-gray-200 shadow-xl rounded-lg p-2 min-w-[140px] animate-fade-in flex flex-col gap-2">
                                                <p className="text-[10px] text-gray-500 text-center font-medium border-b border-gray-50 pb-1">
                                                    Restaurar padrão?
                                                </p>
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <button 
                                                        ...
                                                        className="flex-1 flex items-center justify-center gap-1 bg-gray-100 text-gray-600 p-1 rounded hover:bg-gray-200 transition-colors text-[10px] font-bold"
                                                    >
          ...
          {hasHiddenFields && (
            <div className="mt-4 flex justify-center">
              <button 
                ...
                className="flex items-center gap-1 text-[10px] font-bold text-gray-400 uppercase tracking-widest hover:text-black transition-colors py-1.5 px-3 rounded-md hover:bg-gray-50"
              >
                {isExpanded ? (
                  <>
        ...
        {showSaveConfirmation && (
            <div className="absolute inset-0 z-50 bg-white/95 backdrop-blur-sm flex items-center justify-center p-4 animate-fade-in rounded-xl">
                <div className="text-center max-w-[280px] animate-scale-in">
                    <div className="w-10 h-10 bg-amber-50 rounded-full flex items-center justify-center mx-auto mb-3 text-amber-500 border border-amber-100">
                        <AlertTriangle className="w-5 h-5" />
                    </div>
                    <h4 className="text-sm font-bold text-gray-900">Salvar Credenciais?</h4>
                    ...
                    <div className="flex gap-2 justify-center">
                        <button 
                            ...
                            className="px-3 py-1.5 bg-gray-100 text-gray-600 text-xs font-bold rounded-md hover:bg-gray-200 transition-colors uppercase tracking-wide"
                        >
                            Cancelar
                        </button>
                        <button 
      ...
      {activeHelp && (
        <div 
          className="fixed inset-0 z-[100] flex items-center justify-center bg-white/90 backdrop-blur-sm p-4 animate-fade-in"
          onClick={() => setActiveHelp(null)}
        >
          <div 
            className="bg-white rounded-xl shadow-2xl border border-gray-100 w-full max-w-sm overflow-hidden animate-scale-in"
            onClick={(e) => e.stopPropagation()}
          >
             <div className="p-4 border-b border-gray-100 flex justify-between items-center">
                <h3 className="font-bold text-sm text-gray-900 flex items-center gap-2 uppercase tracking-wide">
             ...
             <div className="p-6">
                <p className="text-xs font-bold text-gray-400 mb-1 uppercase tracking-wide">Campo: {activeHelp.title}</p>

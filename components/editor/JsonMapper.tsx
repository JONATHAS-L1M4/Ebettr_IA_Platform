        ...
        <div className="select-none">
            <div 
                ...
                className={`
                    ...
                    ${level > 0 ? 'ml-3 border-l-2 border-l-gray-100 rounded-l-none pl-2' : ''}
                    ${isUsed 
                        ? 'opacity-50 cursor-not-allowed bg-gray-50' 
                        : isExpandable 
                            ? 'cursor-pointer hover:bg-gray-50 text-gray-800 font-semibold'
                            : 'cursor-pointer hover:bg-blue-50 hover:border-blue-100 hover:text-blue-700 hover:shadow-sm'
                    }
                `}
                title={path}
            >
                <div className="flex items-center gap-1.5 overflow-hidden">
                    ...
                    <span className={`font-mono truncate ${isUsed ? 'line-through text-gray-400' : 'text-gray-700'} ${isExpandable ? 'text-[11px]' : 'font-medium'}`}>
                        {label}
                    </span>
                    {!isExpandable && <span className="text-gray-300">:</span>}
                </div>
                
                <div className="flex items-center gap-2 shrink-0">
                    {displayValue && (
                        <span className={`font-mono text-[10px] truncate max-w-[120px] ${isUsed ? 'text-gray-300' : 'text-gray-500'} ${typeof value === 'boolean' ? (value ? 'text-green-600' : 'text-red-500') : ''}`}>
                            {displayValue}
                        </span>
                    )}
                    {isUsed && <Lock className="w-3 h-3 text-gray-300" />}
                </div>
            </div>
            ...
            {!parsedJson || !data ? (
                <div className="flex flex-col h-full items-center justify-center text-center mt-10">
                    <div className="text-gray-400">
                        ...
                        <p className="text-xs font-medium text-gray-500">Aguardando dados...</p>
                    </div>
                </div>
            ) : (
                /* Grid de Cards */
                <div className="flex flex-col gap-4">
                    {parsedJson.nodes.map((node: any, index: number) => {
                        ...
                        return (
                            <div key={index} className="bg-white border border-gray-200 rounded-lg shadow-sm animate-fade-in group transition-colors hover:border-gray-300">
                                ...
                                <div className="px-4 py-3 border-b border-gray-100 flex items-center justify-between bg-gray-50/50 rounded-t-lg">
                                    <div className="flex items-center gap-3">
                                        <div className="w-6 h-6 bg-white border border-gray-200 rounded flex items-center justify-center text-gray-500 shadow-sm">
                                            ...
                                            <div className="flex items-center gap-1.5">
                                                ...
                                                {node.notes && (
                                                    <Dialog.Root>
                                                                ...
                                                                <Tooltip.Trigger asChild>
                                                                    <Dialog.Trigger asChild>
                                                                        <button className="flex items-center outline-none rounded-full transition-colors">
                                                                            <CircleHelp className="w-3.5 h-3.5 text-gray-400 hover:text-gray-900 cursor-pointer" />
                                                                        </button>
                                                                    </Dialog.Trigger>
                                                                </Tooltip.Trigger>
                                                                <Tooltip.Portal>
                                                                    <Tooltip.Content 
                                                                        sideOffset={8} 
                                                                        className="w-max max-w-[200px] bg-gray-900 text-white text-[10px] px-2 py-1 rounded shadow-md z-[9999] animate-in fade-in zoom-in-95"
                                                                    >
                                                        ...
                                                        <Dialog.Portal>
                                                            <Dialog.Overlay className="fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-[10000]" />
                                                            <Dialog.Content className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white rounded-xl shadow-xl z-[10001] overflow-hidden border border-gray-200">
                                                                <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
                                                                    <div className="flex items-center gap-3">
                                                                        ...
                                                                        <div>
                                                                            <h3 className="text-sm font-semibold text-gray-900">Detalhes do Campo</h3>
                                                                            <p className="text-xs text-gray-500">{node.name || 'Sem nome'}</p>
                                                                        </div>
                                                                    </div>
                                                                ...
                                                                <div className="p-5">
                                                                    ...
                                                                    <div className="mt-5 flex justify-end">
                                                                        <Dialog.Close asChild>
                                                                            <button className="px-4 py-2 bg-white border border-gray-200 hover:bg-gray-50 text-gray-700 text-sm font-medium rounded-lg transition-colors">
                                                                                Entendi
                                                                            </button>
                                                                        </Dialog.Close>
                                                                    </div>
                                    ...
                                    <span className="text-[9px] font-mono font-medium text-gray-400 bg-white border border-gray-200 px-1.5 py-0.5 rounded">
                                        #{index}
                                    </span>
                                </div>
                                ...
                                <div className="p-3">
                                    ...
                                    {hasParams ? (
                                        ...
                                        <div className="py-2 text-center">
                                            <p className="text-[10px] text-gray-400 italic">Sem parâmetros visíveis.</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    {parsedJson.nodes.length === 0 && (
                        <div className="text-center py-10">
                            <div className="bg-amber-50 border border-amber-100 p-4 rounded-lg inline-block text-left max-w-[250px]">
                                <p className="text-xs font-bold text-amber-700 mb-1 flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Atenção</p>
                                <p className="text-[10px] text-amber-600">Nenhum nó marcado com <strong>"Notes In Flow"</strong> foi encontrado no workflow.</p>
                            </div>
                        </div>
                    )}
                </div>

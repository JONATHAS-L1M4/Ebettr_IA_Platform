  ...
  return (
      <div className="animate-fade-in max-w-3xl mx-auto pb-12">
        <div className="flex items-center gap-4 mb-8">
           <button onClick={onCancel} className="w-8 h-8 rounded-full border border-border flex items-center justify-center hover:border-ring/40 hover:text-foreground text-muted-foreground transition-colors bg-card">
             <ArrowLeft className="w-4 h-4" />
           </button>
           <div>
             <h1 className="text-xl font-bold text-foreground">Editar Agente</h1>
             <p className="text-sm text-muted-foreground font-light">Atualize a identidade e o workflow do agente.</p>
           </div>
        </div>

        <form onSubmit={handleSaveSettings} className="bg-panel rounded-lg border border-border shadow-sm overflow-visible">
              ...
              <div className="flex flex-col gap-4">
                 ...
                 <div className="flex flex-col gap-1">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-[10px] text-muted-foreground font-mono">{settingsForm.name.length}/60</span>
                    </div>
                 ...
                 <div className="flex flex-col gap-1">
                    <div className="flex justify-between items-center">
                        ...
                        <span className="text-[10px] text-muted-foreground font-mono">{settingsForm.description.length}/300</span>
                    </div>
                 ...
                 <div className="pt-2 flex flex-col gap-4">
                    <div className="flex flex-col gap-1" ref={companyDropdownRef}>
                        ...
                        <div className="relative">
                            <div className="absolute left-3 top-2.5 text-muted-foreground pointer-events-none"><User className="w-4 h-4" /></div>
                            ...
                            {isCompanyDropdownOpen && filteredCompanies.length > 0 && (
                                <div className="absolute top-full left-0 right-0 mt-1 bg-popover border border-border rounded-lg shadow-xl z-50 overflow-hidden animate-scale-in">
                                    {filteredCompanies.map(comp => (
                                        <button key={comp.id} type="button" onClick={() => handleSelectCompany(comp)} className="w-full text-left px-4 py-2 hover:bg-muted/40 text-sm text-foreground flex items-center justify-between transition-colors border-b border-border last:border-0">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <div className="relative">
                                <div className="absolute left-3 top-2.5 text-muted-foreground"><Phone className="w-4 h-4" /></div>
              ...
              <div className="flex flex-col gap-4">
                 ...
                 <div className="flex flex-col gap-1 relative z-50" ref={dropdownRef}>
                    <div className="flex justify-between items-center">
                        ...
                        <button type="button" onClick={() => loadWorkflows(true)} disabled={isLoadingWorkflows} className="text-[10px] text-muted-foreground hover:text-foreground flex items-center gap-1 transition-colors disabled:opacity-50">
                           <RefreshCw className={`w-3 h-3 ${isLoadingWorkflows ? 'animate-spin' : ''}`} /> Atualizar
                        </button>
                    </div>
                    <div className="relative">
                        <input type="text" placeholder="Buscar workflow..." value={settingsForm.workflowName} onChange={(e) => { setSettingsForm(prev => ({ ...prev, workflowName: e.target.value, workflowId: '' })); setIsDropdownOpen(true); }} onFocus={() => setIsDropdownOpen(true)} className={`${inputClass} pr-8 ${settingsForm.workflowId ? 'font-bold text-foreground' : ''}`} />
                        <div className="absolute right-3 top-2.5 text-muted-foreground">{isLoadingWorkflows ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}</div>
                            ...
                            <div className="mt-1 bg-popover border border-border rounded-lg shadow-sm max-h-56 overflow-y-auto animate-scale-in w-full">
                                {filteredWorkflows.length > 0 ? (
                                    filteredWorkflows.map(wf => (
                                        <button key={wf.id} type="button" onClick={() => handleSelectWorkflow(wf)} className="w-full text-left px-4 py-2 hover:bg-muted/40 text-sm text-foreground flex items-center justify-between group transition-colors border-b border-border last:border-0">
                                            <div className="flex flex-col min-w-0 pr-3">
                                                <span className="text-sm text-foreground group-hover:text-foreground font-medium truncate">{wf.name}</span>
                                                <span className="text-[9px] text-muted-foreground font-mono truncate">#{wf.id}</span>
                                            </div>
                                            <span className={`text-[9px] font-bold ${wf.active ? 'text-foreground bg-muted/70' : 'text-muted-foreground bg-muted/40'} px-2 py-0.5 rounded-full uppercase tracking-wider shrink-0`}>
                                                {wf.active ? 'Ativo' : 'Desativado'}
                                            </span>
                                        </button>
                                    ))
                                ) : <div className="px-4 py-3 text-sm text-muted-foreground text-center">Nenhum workflow encontrado.</div>}
                            </div>
                        )}
                    </div>
                    {settingsForm.workflowId && (
                        <div className="mt-2 text-[10px] text-muted-foreground font-mono bg-muted/40 p-2 rounded border border-border">
                            <span className="font-bold text-muted-foreground">ID VINCULADO:</span> {settingsForm.workflowId}
                        </div>
                    )}
                 </div>
                         ...
                         <div className="flex flex-col gap-4 animate-fade-in bg-muted/30 p-4 rounded-lg border border-border">
                              <div className="flex flex-col gap-1">
                                  ...
                                  <div className="relative">
                                      ...
                                      <input type="url" placeholder="https://..." value={settingsForm.ragUploadUrl || ''} onChange={(e) => setSettingsForm({...settingsForm, ragUploadUrl: e.target.value})} className={`${inputClass} pl-9 bg-card`} />
                                  </div>
                              </div>
                              <div className="flex flex-col gap-1">
                    ...
                    {settingsForm.hasTestMode && (
                        <div className="flex flex-col gap-4 animate-fade-in bg-muted/30 p-4 rounded-lg border border-border">
                             <div className="flex flex-col gap-1">
                                 ...
                                 <div className="relative">
                                     <div className="absolute left-3 top-2.5 text-muted-foreground"><Globe className="w-4 h-4" /></div>
                                     <input type="url" placeholder="https://..." value={settingsForm.testWebhookUrl} onChange={(e) => setSettingsForm({...settingsForm, testWebhookUrl: e.target.value})} className={`${inputClass} pl-9 bg-card`} />
                                 </div>
                             </div>
                        </div>
                    )}
           ...
           <div className="p-6 bg-muted/40 border-t border-border flex items-center justify-end gap-3">
               <button type="button" onClick={onCancel} className="px-4 py-2 text-sm text-muted-foreground hover:text-foreground font-medium transition-colors">Cancelar</button>
               <button type="submit" disabled={!settingsForm.workflowId || isSaving} className="px-6 py-2 bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-bold rounded-md transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        {onDelete && isAdmin && (
            <div className="mt-8 pt-8 border-t border-border animate-fade-in">
                ...
                <div className="border border-destructive/30 bg-destructive/10 rounded-lg p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h4 className="text-sm font-bold text-foreground">Excluir Agente</h4>
                        <p className="text-xs text-muted-foreground mt-1">Esta ação excluirá permanentemente o agente <strong>{agent.name}</strong>.</p>
                    </div>
                    <button type="button" onClick={onDelete} className="px-4 py-2 bg-card border border-destructive/40 text-destructive text-xs font-bold rounded-md hover:bg-destructive hover:text-destructive-foreground transition-all flex items-center justify-center gap-2 shrink-0">

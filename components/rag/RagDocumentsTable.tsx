...
export const RagDocumentsTable: React.FC<RagDocumentsTableProps> = ({ 
  ...
  if (documents.length === 0 && !isLoading) {
    return (
        <div className="p-12 border border-dashed border-gray-200 rounded-xl bg-gray-50/50 text-center">
      ...
      <tr key={`skeleton-${index}`} className="animate-pulse border-b border-gray-50">
        ...
        <td className="px-5 py-3">
          <div className="flex items-center gap-3">
            <div className="w-6 h-6 bg-gray-100 rounded shrink-0"></div>
            <div className="h-3 w-48 sm:w-64 bg-gray-100 rounded"></div>
          </div>
        </td>
        ...
        <td className="px-5 py-3 hidden md:table-cell">
          <div className="h-4 w-12 bg-gray-100 rounded border border-gray-200"></div>
        </td>
        <td className="px-5 py-3">
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg shadow-sm overflow-hidden">
      <div className="p-5 border-b border-gray-100 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
            <h3 className="text-base font-bold text-gray-900">Documentos</h3>
            ...
            <p className="text-[10px] text-gray-500 font-light sm:hidden mt-0.5">{filteredDocuments.length} documentos encontrados</p>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3 w-full sm:w-auto">
            {selectedIds.length > 0 && onBulkDelete && (
                <button
                    ...
                    className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wide transition-all shadow-sm border bg-red-50 text-red-600 border-red-200 hover:bg-red-600 hover:text-white hover:border-red-700 disabled:opacity-50 w-full sm:w-auto"
                >
            ...
            <div className="relative w-full sm:w-64">
              ...
              <input
                ...
                placeholder="Buscar..."
                className="block w-full pl-9 pr-8 py-2.5 border border-gray-200 rounded-lg text-xs bg-white focus:outline-none focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 transition-all shadow-sm"
              />
              {searchTerm && (
                <button
            ...
            <div className="flex items-center gap-2 w-full sm:w-auto justify-between sm:justify-end">
                <select 
                  ...
                  className="px-3 py-2.5 text-xs font-bold text-gray-500 uppercase tracking-wide rounded-lg bg-white shadow-sm border border-gray-200 focus:outline-none focus:ring-1 focus:ring-emerald-500 cursor-pointer"
                >
                ...
                <div className="flex items-center gap-2">
                    <button 
                        ...
                        className="px-4 py-2.5 text-xs font-bold text-gray-500 hover:text-black uppercase tracking-wide transition-all disabled:opacity-30 disabled:cursor-not-allowed rounded-lg bg-white shadow-sm border border-gray-200"
                    >
                        Voltar
                    </button>
                    <button 
          ...
          <tbody className="divide-y divide-gray-100 text-sm">
            {isLoading ? renderSkeletonRows() : (
              paginatedDocuments.length > 0 ? paginatedDocuments.map((doc) => (
                <tr key={doc.id} className={`hover:bg-gray-50 transition-colors group ${selectedIds.includes(doc.id) ? 'bg-emerald-50/30' : ''}`}>
                  ...
                  <td className="px-5 py-3 font-medium text-gray-900">
                    <div className="flex items-center gap-3">
                      <div className="w-6 h-6 rounded bg-gray-100 text-gray-500 flex items-center justify-center shrink-0">
                  ...
                  <td className="px-5 py-3 whitespace-nowrap text-gray-600 hidden sm:table-cell">
                      <span className="text-[10px] font-medium text-gray-500 uppercase tracking-wide">
                          {doc.blobType}
                      </span>
                  </td>
                  <td className="px-5 py-3 whitespace-nowrap font-mono text-[11px] text-gray-600 hidden md:table-cell">
                  ...
                  <td className="px-5 py-3 whitespace-nowrap text-center">
                    <button 
                        ...
                        className="p-1.5 text-gray-400 hover:text-black hover:bg-gray-200 rounded-md transition-all inline-flex items-center justify-center mr-1"
                    ...
                    <button 
                        ...
                        className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-md transition-all inline-flex items-center justify-center"
                        title="Excluir"
                    >
